...
- **🎬 前端全屏沉浸版 + 语音对话流合并（2026-08-21 老板指示，已完成）**：整页播放数字人视频（avatar-stage 全屏 cover）+ 聊天框半透明玻璃浮层（chat-overlay，rgba+blur 26px）嵌入视频上；品牌栏半透明悬浮。**语音字幕与文字聊天合并为统一对话流**（onSubtitle→live assistant 消息带 ● 呼吸点、onUserTranscript→user 消息、时间排序）+ **折叠/展开**（默认最近 3 条，点"全部对话 N"展开 78vh）。四条语音 bug 已修：① vite `publicDir` 直服素材（dev 不依赖后端）② StrictMode updater 副作用吞 live 消息（id 生成移出 updater）③ audio_transcript.delta 是逐字增量需 **append 非覆盖** ④ 对话顺序 **用 VAD speech_started 锚点**（user transcript final 晚于 asst delta 1-2s，Date.now() 不可靠）。一键启动 `start-dev.bat`（后端已跑跳过+清 3000 残留+健康检查等就绪+自动开 Edge）；vite `strictPort` 固定 5173 + `free-port.cjs` 参数化。
- **🎞 素材浏览器兼容规范（2026-08-21 定）**：HY-Image VideoGen 产出的 mp4 是 **MPEG-4 Visual (mp4v)**——浏览器（Edge/Chrome/Firefox）不支持，只有 WorkBuddy 内置 Chromium 能放；**新素材必须转码 H.264**：`ffmpeg -y -i in.mp4 -c:v libx264 -preset medium -crf 23 -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart out.mp4`（ffmpeg 用 imageio-ffmpeg 自带二进制，路径记于工作日志）；原件备份 `assets/avatars/clips/_originals_mp4v/`。
...
- [x] **前端全屏沉浸版 + 语音对话流合并（2026-08-21 完成）**：`client/src/App.tsx` + `index.css`——整页数字人视频（avatar-stage）+ 半透明聊天浮层（chat-overlay）+ 语音/文字合并对话流 + 折叠/展开；`client/vite.config.ts`（publicDir 直服素材 + strictPort 5173）；`client/scripts/free-port.cjs`（参数化）；根 `start-dev.bat`（一键启动）；素材 6 个转码 H.264（mp4v 浏览器不支持）；四条语音链路 bug 修复（见上方"当前任务定位"）
- [ ] P5 体验优化 + 收尾

---
*最后更新：2026-08-21（前端全屏沉浸版 + 语音对话流合并 + 四条语音链路 bug 修复 + 素材 H.264 转码规范 + 一键启动脚本）*
