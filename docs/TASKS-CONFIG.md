...
| 项 | 内容 |
|----|------|
| **执行入口** | 老板把**说明文档**交给 Claude Code（`docs/reviews/CC-01_说明文档.md` / `CC-02_说明文档.md`，自包含可直接执行） |
| **反馈机制** | 完成后填写**反馈文档**（`docs/reviews/CC-01_反馈文档.md` / `CC-02_反馈文档.md`，模板） |
| **职责边界** | 深度分析（审查/审计），只诊断不改码；产出报告到 `docs/reviews/` |
| **⏰ 执行时机** | 老板 2026-08-09 拍板：**最后再做**（M5 阶段；M5-02~05 已完成，v0.1.0 已发布，现在正是时机） |

**任务清单**：

| ID | 任务 | 优先级 | 状态 | 验收标准 |
|----|------|--------|------|----------|
| CC-01 | 已交付代码审查 | P1 | ✅ | 覆盖全部 36 个核心文件，报告含分级问题+合规检查+契约对照（2026-08-11 完成：48 问题，整改见 CC-01-02 整改文档） |
| CC-02 | 依赖与安全审计 | P1 | ✅ | 依赖清单分析 + npm audit + 配置/密钥检查（2026-08-11 完成：依赖达标，漏洞已随整改修复） |
| CC-03 | 测试缺陷整改 | P1 | ✅ | 整改单模块测试 3 缺陷（DEF-A-01/02、DEF-V-01）+ 补 M-P/M-C 测试文件（2026-08-11 完成：3 缺陷关闭、M-P 9/9 + M-C 9/9、全量回归绿） |
| CC-04 | H6 路径校验修复 | P1 | ✅ | `resolveIdDir` 校验规范化 base（2026-08-11 完成：`startsWith(resolve(personasDir))` + 大小写归一，实测 getPersona 通过） |
