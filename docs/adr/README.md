...
### Status
~~Accepted~~ → **Deprecated（老板 2026-08-09 撤销，环境搭建恢复执行）**

### Context
- 老板原话："环境不用搭建了，不是你负责的事情，本任务永久暂停环境搭建。"（2026-08-09 早先）
- **2026-08-09 晚些时候老板撤销该决策**："环境搭建永久暂停这个去掉。"
...
**已撤销**：环境搭建恢复执行——子任务可按需执行 npm/pnpm install、依赖安装、工具链配置等环境类操作。DESIGN §16 依赖策略与 DEPENDENCIES.md 恢复参考价值。

### Consequences
- ✅ 子任务可自主安装依赖并跑通环境，交付"可运行"的代码
- ✅ 联调可在项目内完成，不再依赖外部环境
- ⚠️ 依赖安装需遵守轻量化约束（运行时 5-6 个纯 JS 依赖，零原生编译，ADR-007）
